...
#endif
{
    ...
    public static async Task<KmlLabelStyle> StaticReadTagAsync(XmlReader reader)
    {
        ...
        while (await reader.MoveToContentAsync().ConfigureAwait(false) is not XmlNodeType.EndElement and not XmlNodeType.None)
        {
            ...
            if (reader.NodeType == XmlNodeType.Element)
            {
                if (Helpers.CheckElementName(reader, "scale", Namespaces.Kml, alreadyLoaded))
                    o.Scale = await Helpers.ReadElementDoubleAsync(reader, alreadyLoaded).ConfigureAwait(false);
...
#warning todo unknown elements
    ...
    public static async Task StaticWriteTagAsync(XmlWriter writer, KmlLabelStyle o, XmlNamespaceManager? ns = null, KmlWriteOptions? options = null)
    {
    ...
    public override Task WriteTagAsync(XmlWriter writer, KmlLabelStyle o, XmlNamespaceManager? ns = null, KmlWriteOptions? options = null)
        => StaticWriteTagAsync(writer, o, ns, options);

    public override Task<KmlLabelStyle> ReadTagAsync(XmlReader reader)

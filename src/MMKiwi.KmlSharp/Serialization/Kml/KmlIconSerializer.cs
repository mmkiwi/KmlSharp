...
internal class KmlIconSerializer : ISerializationHelper<KmlIcon>
#if NET7_0_OR_GREATER
    , ISerializationHelperStatic<KmlIcon>
#endif
{
    public static string Namespace => Namespaces.Kml;
    public static string Tag => "Icon";

    public static async Task<KmlIcon> ReadTagAsync(XmlReader reader, CancellationToken ct = default)
    {
        ...
        while (await reader.MoveToContentAsync().ConfigureAwait(false) is not XmlNodeType.EndElement and not XmlNodeType.None)
        {
            ...
            {
                if (HelpExtensions.CheckElementName(reader, "href", Namespaces.Kml, alreadyLoaded))
    ...
    public static async Task WriteTagAsync(XmlWriter writer, KmlIcon o, XmlNamespaceManager? ns = null, KmlWriteOptions? options = null, CancellationToken ct = default)
    {
    ...
    Task ISerializationHelper<KmlIcon>.WriteTagAsync(XmlWriter writer, KmlIcon o, XmlNamespaceManager? ns, KmlWriteOptions? options, CancellationToken ct)
        => WriteTagAsync(writer, o, ns, options, ct);

    Task<KmlIcon> ISerializationHelper<KmlIcon>.ReadTagAsync(XmlReader reader, CancellationToken ct)
        => ReadTagAsync(reader, ct);

    string ISerializationHelper<KmlIcon>.Tag => Tag;
    string ISerializationHelper<KmlIcon>.Namespace => Namespace;
}